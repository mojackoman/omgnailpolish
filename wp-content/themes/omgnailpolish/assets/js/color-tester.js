...
$.buzz.events = function () {
	...
	$('.select-feet').on('click', function(e) {
		...
		$('.feet').fadeIn(250, function(){
			...
			$('.feet img[class="layer-04"]').css({'opacity' : '0'});
		});
...
$.buzz.canvass = {
	...
	init: function () {
		$.buzz.canvass.resize();
		$('.color-tester .canvass .hand .layer-04').css('opacity', 0.01);
...
$.buzz.preloads = function() {
	...
	$('.color-container a').each(function(i, obj){
		if(regExp.exec($(this).attr('canvass-data'))){
			var tmpcan = regExp.exec($(this).attr('canvass-data'));
			tmp[ctrcan] = tmpcan[1];
			ctrcan++
		}
		
		if(regExp.exec($(this).attr('data-hand'))){
			var tmpcan = regExp.exec($(this).attr('data-hand'));
			hand[ctrhand] = tmpcan[1];
			ctrhand++
		}		
		
		if(regExp.exec($(this).attr('data-feet'))){
			var tmpcan = regExp.exec($(this).attr('data-feet'));
			feet[ctrfeet] = tmpcan[1];
			ctrfeet++
		}				
		//console.log();
